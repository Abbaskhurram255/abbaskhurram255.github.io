...
const isWeekday = (d = new Date()) => { return d.getDay() % 6 !== 0;}

//fn that tests if it's weekend today and returns boolean value as a result
const isWeekend = (d = new Date()) => { return d.getDay() % 6 === 0; }
...
const milesToKm = miles => { return miles * 1.609344; }

const kmToMiles = km => { return km * 0.621371; }
...
var bday = prompt(
  ...
  const q1 =
   /what'?s?( is)? (up|popping)/i,
   q2 = /who are you/i,
   q3 =
   /(who am I)|(my (info|bio))|(what( do)? you know about me)/i,
   q4 =
   /(how are you)|(how('?ve?| have) you been)/i,
   ...
   if (q1.test(q)) {
    ...
    let age = calc_age(new Date(
     ...
     msg += "</li></ul></em>";
         $output.html(msg);
         console.log(msg); 
      if (/your (name|age|birthday):/gmi.test(msg)) {
    ...
    if ((bday != null && /^[0-9a-zA-Z(-\.\_\s\/)]+$/gi.test(bday)) && (bday != '' && /^\d{4}[\/.,-\s](\d{1,2}|\b(\w*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*)\b)[\/.,-\s]\d{1,2}$/gi.test(bday))) {
     ...
     log(msg);
     if (isBday(bday)) {
      msg += " (today)<br>Happy birthday, by the way! A gift for you &#127874;";
      }
    } else {
     bday = prompt(
     ...
     if ((bday != null && /^[0-9a-zA-Z(-\.\_\s\/)]+$/gi.test(bday)) && (bday != '' && /^\d{4}[\/.,-\s](\d{1,2}|\b(\w*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*)\b)[\/.,-\s]\d{1,2}$/gi.test(bday))) {
      ...
      bday = prompt(
       "Can't calculate your age without knowing your bday. Please submit your bday first.\nSupported format: YYYY(separator)m(separator)d");
       age = calc_age(new Date(
        bday)); msg = 'You are ' +
       age;
       if (age <= 18) {
...
console.log(msg);
        ...
        if (stonePSWin) { window.focus(); log("Opened StonePaperScissor"); } else { alert('Please turn on popups on this site.'); }
        stopText();
        msg = "";
        $output.html(msg);
      } else if (q30.test(q)) {
        ...
        if (curConvAppWin) { window.focus(); log("Opened TTS"); } else { alert('Please turn on popups on this site.'); }
        stopText();
        msg = "";
        $output.html(msg);
      } else if (q32.test(q)) {
        $('#message').slideFadeToggle(800);
        msg = "This is day " + dayOfYear(new Date()) + " of year " + new Date().getFullYear();
        console.log(msg);
        $output.html(msg);
        $('#message').delay(10000).slideFadeToggle(800);
      } else if (q33.test(q)) {
      ...
      $('#message').slideFadeToggle(800);
      if (isWeekend()) {
        msg = "Yes. Of course, it is! Why don't you go for an adventure or something?";
      } else {
        msg = "According to my intelligence, no, it's not.";
        }
      console.log(msg);
