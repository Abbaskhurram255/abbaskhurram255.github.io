...
const q1 = /what'?s?( is)? (up|popping)/i,
  ...
  q8 = /((what'?s?|when'?s?)( is)?|show) my (birthday|b-day|bday|day)/i,
  q9 = /(how old am I)|((show|what'?s?( is)?) my age)/i,
  q10 = /(call me by another|change my) name/i,
  q11 = /change my (dob|bday|day|birthday)/i,
  q12 = /am I nice/i,
...
function ask() {
  ...
  if (q1.test(q)) {
    ...
    msg += "Here's what I know about you:";
    msg += "<ul><li>Your name: " + userName + "</li>" + "<li>Your birthday: ";
    if (
      bday != null &&
      /^[0-9a-zA-Z(-\.\_\s\/)]+$/gi.test(bday) &&
      bday != "" &&
      /^\d{4}[\/.,-\s](\d{1,2}|\b(\w*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*)\b)[\/.,-\s]\d{1,2}$/gi.test(
        bday
      )
    ) {
      msg += bday;
    ...
    if (
      ...
      bday = toTitleCase(bday);
      msg = `Date set. Your new bday is ${bday}`;
      log(msg);
    }
    $output.html(msg);
    console.log(msg);
    ...
    if (cfm) {
      ...
      if (
        ...
        msg = `&#128076; Success. Your new bday is ${bday}&#9786;`;
        console.log(msg);
      } else {
        bday = prompt(
