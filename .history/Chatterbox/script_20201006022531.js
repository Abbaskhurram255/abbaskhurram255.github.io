...
var bday = prompt(
  ...
  const q1 =
   ...
   q22 = /(music)|(songs?)/gi,
   q23 = /random (stuff|tools|apps)/gi,
   q24 = /(random? (fun|games))|(bored)/gi;
...
console.log(msg);
        ...
        if (randStuffWin) { window.focus(); log("Opened a random app"); } else { alert('Please turn on popups on this site.'); }
        	...
        	let games = ['./tic-tac-toe-master/', './2D-Breakout-Game-JavaScript-master/', './Tetris-JavaScript-master/', './Snake-JavaScript-master/', './FlappyBird-JavaScript-master/', './JavaScript-Pacman-master/', './hangman-master/'];
        let gWin = window.open(games[Math.floor(Math.random() * randStuff.length)], '_blank');
