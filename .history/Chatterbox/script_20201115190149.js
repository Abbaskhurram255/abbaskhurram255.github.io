  ...
  if (q1.test(q)) {
	...
	*/
    ...
    $output.html(msg);
      ...
      let seriesApps = ["../tv-series-app/", "../moviedb/"]
      let seriesAppsWin = window.open(
        seriesApps[Math.floor(Math.random() * seriesApps.length)],
        "_blank"
      );
      if (seriesAppsWin) {
        window.focus();
        log("Launched an app for TV Series data");
      } else {
        alert("Please enable popups for this site!");
      }
      stopText();
