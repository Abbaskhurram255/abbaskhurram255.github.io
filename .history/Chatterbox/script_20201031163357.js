...
window.onload = () => {
  ...
  tippy(tippy1, {
    content:
      "Try asking me for show <strong onclick=\"$('#searchInput').val($(this).html());\" onmouseover=\"$(this).css('cursor', 'pointer');\">weather</strong> or to open <strong onclick=\"$('#searchInput').val($(this).text());\" onmouseover=\"$(this).css('cursor', 'pointer')\">calendar</strong>, <strong onclick=\"$('#searchInput').val($(this).text())\" onmouseover=\"$(this).css('cursor', 'pointer')\">show exchange rates</strong>, or <strong onclick=\"document.querySelector('#searchInput').value = 'whats my ' + this.innerHTML;\" onmouseover=\"$(this).css('cursor', 'pointer')\">your age</strong>",
...
const q1 = /what'?s? (is)? (up|popping)/i,
  ...
  q43 = /-deleted-maps.-add-another-app-on-its-behalf/i,
...
function ask() {
  ...
  if (q1.test(q)) {
    ...
    if (randomQuoteWin) {
      window.focus();
      msg = "Launched Quotes application";
      $output.html(msg);
      log(msg);
    } else {
      alert("Please enable popups for this site!");
