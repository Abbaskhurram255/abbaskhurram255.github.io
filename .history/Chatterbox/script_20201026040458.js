...
function ask() {
  ...
  if (q1.test(q)) {
    ...
    msg += `. Want it changed? <a onclick="$('#searchInput').val('Change my name');" onmouseover="$(this).css('cursor', 'pointer')" style="color:rgba(0,0,255,0.9);text-decoration:underline;">Click here</a>`;
      ...
      msg += "not saved yet";
      msg += `. <a onclick="$('#searchInput').val('Change my name');" onmouseover="$(this).css('cursor', 'pointer')" style="color:rgba(0,0,255,0.9);text-decoration:underline;">Click here to (re)submit it, your name</a>`;
    }
    $output.html(msg);
    console.log(msg);
    $("#message").delay(10000).slideFadeToggle(800);
